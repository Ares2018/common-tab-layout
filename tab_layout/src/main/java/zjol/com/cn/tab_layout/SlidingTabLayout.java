...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mTextUnSelectSize = ta.getDimension(R.styleable.SlidingTabLayout_tl_textUnselectSize, mTextsize);
        mTextBold = ta.getInt(R.styleable.SlidingTabLayout_tl_textBold, TEXT_BOLD_NONE);
        mTextAllCaps = ta.getBoolean(R.styleable.SlidingTabLayout_tl_textAllCaps, false);

        mAlignCenter = ta.getBoolean(R.styleable.SlidingTabLayout_tl_tab_align_center, false);
    ...
    private void addTab(final int position, String title, View tabView) {
        ...
        if (tv_tab_title != null) {
            if (title != null) {
                ...
                while(matcher.find()) {
                    TypefaceSpan monospace = new TypefaceSpan("serif");
                    int start=matcher.start();
                    spannableString.setSpan(monospace, start, start + 1, Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
                }
                tv_tab_title.setText(spannableString);
            }
        }
